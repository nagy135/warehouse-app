...
export default function ExitProductModal({
  ...
  const notMovedSelectedProductStoragesCount =
    selectedProductStorages?.filter((storage) => storage.state === 'none')
      .length || 0;
  ...
  const {
    modal: ProductSkuNotFoundModal,
    setOpen: openProductSkuNotFoundModal,
  ...
  const {
    modal: StorageSkuNotFoundModal,
    setOpen: openStorageSkuNotFoundModal,
  } = useNotificationModal({
    variant: 'danger',
  ...
  const modalText = useMemo(() => {
    switch (step) {
      case ExitProductStepEnum.SCAN_PRODUCT:
        return {
          title: t('exit-product-modal.scanning-product'),
          body: isFocused && (
            <Scanner
              ...
              onScan={(skuCode) => {
                const storages = productStoragesOnScannedLocation?.filter(
      ...
      case ExitProductStepEnum.SET_COUNT:
        return {
          title: `${t('exit-product-modal.product')}: ${selectedProductStorages?.[0].productSkuVariant.name}`,
          body: (
            <Input
      ...
      case ExitProductStepEnum.SCAN_STORAGE:
        return {
          title: t('exit-product-modal.scanning-storage'),
          body: isFocused && (
            <Scanner
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle className="h-14">
            <View>
              <Text className="text-lg font-extrabold">{modalText?.title}</Text>
            </View>
          </AlertDialogTitle>
          <AlertDialogDescription>
            <View className="h-12">{modalText?.body}</View>
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          {step === ExitProductStepEnum.SET_COUNT && (
            <AlertDialogCancel
              onPress={() => {
