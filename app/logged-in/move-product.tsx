...
export default function MoveProductPage() {
  ...
  const [step, setStep] = useState<MoveProductStepEnum>(
    MoveProductStepEnum.SCAN_PRODUCT,
  );
  ...
  const {
    modal: storageFromNotFoundModal,
    setOpen: openStorageFromNotFoundModal,
  } = useNotificationModal({
    variant: 'danger',
    title: t('move-section.storage-not-found'),
    description: t('move-section.product-storage-not-found-description', {
      product: product?.name,
    }),
  });

  const {
    modal: storageNotEnoughItemsModal,
    setOpen: openStorageNotEnoughItemsModal,
  } = useNotificationModal({
    variant: 'danger',
    title: t('move-section.not-enough-items'),
    description: t('move-section.not-enough-items-description', {
      product: product?.name,
    }),
  });

  const { modal: storageToNotFoundModal, setOpen: openStorageToNotFoundModal } =
    useNotificationModal({
      variant: 'danger',
      title: t('move-section.storage-not-found'),
  ...
  const { mutate: mutateTransferProductStorages, error } =
    useTransferProductToStorage({
      onSuccessCallback: () => {
        resetProcess();
        openSuccessModal();
      },
  ...
  const { modal: errorModal, setOpen: openErrorModal } = useNotificationModal({
    ...
    description: t('move-section.transfer-product-error-description', {
      error,
    }),
  });
  ...
  return (
    <View className="flex-1 items-center justify-center gap-5 bg-secondary/30 p-6">
      <View className="flex w-full justify-center gap-3">
        {isFocused && step === MoveProductStepEnum.SCAN_PRODUCT && (
          <Scanner
            label={t('move-section.scan-product')}
            mockData="bluepancakes1k123"
            onScan={(data) => {
              mutateCheckProductExits({ sku: data })
                .then((data) => {
                  setProduct(data);
        ...
        {product && (
          <View>
            <Text className="text-xl">
              {`${t('move-section.product')}: `}
              <Text className="text-xl font-bold">{product.name}</Text>
            </Text>
          </View>
        )}
        {isFocused && step === MoveProductStepEnum.SCAN_STORAGE_FROM && (
          <Scanner
            label={t('move-section.scan-original-storage')}
            mockData="oldbrokenpallet123"
            onScan={(data) => {
              const storage = product?.productStorages.find(
                (productStorage) => productStorage.storage.sku === data,
              );
              if (storage) {
        ...
        {productCount && (
          <View>
            <Text className="text-xl">
              {`${t('move-section.count')}: `}
              <Text className="text-xl font-bold">{productCount}</Text>
            </Text>
          </View>
        )}
        {isFocused && step === MoveProductStepEnum.SCAN_STORAGE_TO && (
          <Scanner
        ...
        {toStorage && (
          <View>
            <Text className="text-xl">
              ...
              <Text className="text-xl font-bold">{toStorage.name}</Text>
            </Text>
          </View>
        )}
        <View className="mt-5 flex flex-row justify-center gap-3">
          {product && fromStorage && toStorage && (
            <Button
              variant="secondary"
              ...
              onPress={() => {
                const productStorageIds =
                  product?.productStorages
                    .filter(
                      (productStorage) =>
                        productStorage.storage.sku === fromStorage.storage.sku,
                    )
                    .slice(0, productCount)
                    .map((item) => item.id) || [];
                mutateTransferProductStorages({
                  productSkuVariantIds: productStorageIds,
                  toStorageSKU: toStorage.sku,
                });
              }}
            >
      ...
      <CountModal
        ...
        onConfirm={(count) => {
          if (count) {
            const storage = product?.productStorages.filter(
              (productStorage) =>
                productStorage.storage.sku === fromStorage?.storage.sku,
            );
