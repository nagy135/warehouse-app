...
import { useEntryStateContext } from '../contexts/entryStateContext'
    ...
    const groupedProductStorages = useMemo(() => {
        return Object.entries(grouped).map(([_, groupOfProductStorages]) => {
            const uniqueProductStorage: GroupedProductStorage = {
                ...
                count: groupOfProductStorages.filter((storage) => storage.state === 'counted' || storage.state === 'moved').length,
                countedCount: groupOfProductStorages.reduce((prev, next) => prev + (next.state === 'moved' ? 1 : 0), 0),
