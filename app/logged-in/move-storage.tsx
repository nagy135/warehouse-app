...
export default function MovePositionPage() {
  ...
  const [step, setStep] = useState<MoveStorageStepEnum>(MoveStorageStepEnum.SCAN_STORAGE);


  const { modal: storageNotFoundModal, setOpen: openStorageNotFoundModal } = useNotificationModal({
    variant: 'danger',
    title: t('move-section.storage-not-found'),
    description: t('move-section.storage-to-not-found'),
  })

  const { modal: positionNotFoundModal, setOpen: openPositionNotFoundModal } = useNotificationModal({
    variant: 'danger',
    title: t('move-section.position-not-found'),
    description: t('move-section.position-not-found-description'),
  })

  const { modal: positionNotEmptyModal, setOpen: openPositionNotEmptyModal } = useNotificationModal({
    variant: 'danger',
  ...
  const { modal: errorModal, setOpen: openErrorModal } = useNotificationModal({
    ...
    description: t('move-section.transfer-storage-error-description', { error }),
  });

  return (
    <View className="flex-1 justify-center items-center gap-5 p-6 bg-secondary/30">
      <View className="flex justify-center gap-3 w-full">
        {isFocused && step === MoveStorageStepEnum.SCAN_STORAGE && <Scanner
        ...
        {storage && (
          <View>
            <Text className="text-xl">{`${t('move-section.storage')}: `}<Text className="text-xl font-bold">{storage.name}</Text></Text>
          </View>
        )}
        {isFocused && step === MoveStorageStepEnum.SCAN_POSITION && <Scanner
          label={t('move-section.scan-new-position')}
          mockData="randomemptyspot123"
          onScan={(data) => {
            mutateCheckPositionExits({ sku: data }).then((data) => {
        ...
        {position && (
          <View>
            <Text className="text-xl">{`${t('move-section.position')}: `}<Text className="text-xl font-bold">{position.name}</Text></Text>
          </View>
        )}
